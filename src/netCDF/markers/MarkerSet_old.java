...
public class MarkerSet_old {
    ...
    public MarkerSet_old(int studyId, int matrixId) throws IOException{
    ...
    public LinkedHashMap getChrInfoSetLHM() {
        ...
        try{
            ...
            var = ncfile.findVariable(constants.cNetCDF.Variables.VAR_CHR_INFO); //Nb of markers, last physucal position, start index number in MarkerSet_old
    ...
    public LinkedHashMap readAllMarkersGTsForCurrentSampleToLHM(NetcdfFile ncfile, LinkedHashMap rdLhm, int sampleNb) throws IOException{
        Variable genotypes = ncfile.findVariable(constants.cNetCDF.Variables.VAR_GENOTYPES);
...
//                                                                 + "0:"+(varShape[2]-1)+":1)");
    ...
    public LinkedHashMap fillStrandedBasesToLHMArray(LinkedHashMap rdLhm, String strandedBases) throws IOException{
        ...
        try {
            ncfile = NetcdfFile.open(matrixMetadata.getPathToMatrix());
            Variable varStrands = ncfile.findVariable(constants.cNetCDF.Variables.VAR_GT_STRAND);
            Variable varStrandedBases = ncfile.findVariable(strandedBases);

            if (null == varStrandedBases) {return dictionaryLhm;}
            if (null == varStrands) {return dictionaryLhm;}
            try {
    ...
    public LinkedHashMap fillMarkerSetLHMWithChrAndPos(NetcdfFile ncfile) {
        ...
        try {
            markerSetSize = varShape[0];
            if(dataType==DataType.INT){
                ArrayInt.D1 markerSetAI = (ArrayInt.D1) var.read("(0:"+(markerSetSize-1)+":1)");
    ...
    /**
     * This fillALWithVariable never used
     * @deprecated
     */
    public ArrayList fillALWithVariable(NetcdfFile ncfile, String variable){
        ...
        try {
            ...
            if(dataType==DataType.DOUBLE){
                if(varShape.length==1){
                    ArrayDouble.D1 markerSetAF = (ArrayDouble.D1) var.read("(0:"+(markerSetSize-1)+":1)");
                    al = netCDF.operations.Utils.writeD1ArrayDoubleToAL(markerSetAF);
                }
                if(varShape.length==2){
                    ArrayDouble.D2 markerSetAF = (ArrayDouble.D2) var.read("(0:"+(markerSetSize-1)+":1, 0:"+(varShape[1]-1)+":1)");
                    al = netCDF.operations.Utils.writeD2ArrayDoubleToAL(markerSetAF);
                }
            }

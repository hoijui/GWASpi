...
import constants.cNetCDF.Defaults.GenotypeEncoding;
/**
 *
...
public class Utils {
    ...
    public static Object[] fixPlusAlleles(Object[] values, int strandNb, int allelesNb) throws IOException {
            ...
            String cleanAlleles = "";
            if(values[strandNb].equals(constants.cImport.StrandFlags.strandMIN)){ //check if strand is "-"
    ...
    public static GenotypeEncoding detectGTEncoding(LinkedHashMap lhm){
        constants.cNetCDF.Defaults.GenotypeEncoding gtEcoding = constants.cNetCDF.Defaults.GenotypeEncoding.UNKNOWN;
        ...
        if(allAlleles.contains(AlleleBytes.B)){
           gtEcoding = constants.cNetCDF.Defaults.GenotypeEncoding.AB0;
        } else {
            if(allAlleles.contains(AlleleBytes.C) ||
               allAlleles.contains(AlleleBytes.G) ||
               allAlleles.contains(AlleleBytes.T)){
               gtEcoding = constants.cNetCDF.Defaults.GenotypeEncoding.ACGT0;
            } else {
                if(allAlleles.contains(AlleleBytes._3) ||
                   allAlleles.contains(AlleleBytes._4)){
                   gtEcoding = constants.cNetCDF.Defaults.GenotypeEncoding.O1234;
                } else { //ONLY CONTAINS 1,2
                   gtEcoding = constants.cNetCDF.Defaults.GenotypeEncoding.O12;
                }
            }
        }

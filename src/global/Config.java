...
import database.DerbyDBReshaper;
...
public class Config {
    ...
    public static void setConfigValue (String key, Object value) throws IOException {
        ...
        if(key.equals("DataDir")){
            gui.StartGWASpi.config_DataDir=(String) value;
        }
        if(key.equals("GTdir")){
            gui.StartGWASpi.config_GTdir=(String) value;
        }
        if(key.equals("ExportDir")){
            gui.StartGWASpi.config_ExportDir=(String) value;
        }
        if(key.equals("ReportsDir")){
            gui.StartGWASpi.config_ReportsDir=(String) value;
        }
        if(key.equals("LogDir")){
            gui.StartGWASpi.config_LogDir=(String) value;
        }
    }
    ...
    public static String getConfigValue(String key, String defaultV) throws IOException{
        ...
        if(gui.StartGWASpi.guiMode){
            ...
            //CLI MODE
            if(key.equals("DataDir")){
                if(gui.StartGWASpi.config_DataDir != null){prop = gui.StartGWASpi.config_DataDir;} else {prop = defaultV;}
            }
            if(key.equals("GTdir")){
                if(gui.StartGWASpi.config_GTdir != null){prop = gui.StartGWASpi.config_GTdir;} else {prop = defaultV;}
            }
            if(key.equals("ExportDir")){
                if(gui.StartGWASpi.config_ExportDir != null){prop = gui.StartGWASpi.config_ExportDir;} else {prop = defaultV;}
            }
            if(key.equals("ReportsDir")){
                if(gui.StartGWASpi.config_ReportsDir != null){prop = gui.StartGWASpi.config_ReportsDir;} else {prop = defaultV;}
            }
            ...
            if(key.equals("LogDir")){
                if(gui.StartGWASpi.config_LogDir != null){prop = gui.StartGWASpi.config_LogDir;} else {prop = defaultV;}
            }
        }
...
//    }
    ...
    public static void clearConfigFile() throws IOException, BackingStoreException{
        if(startWithGUI){
            ...
            //CLI MODE
            gui.StartGWASpi.config_DataDir=null;
            gui.StartGWASpi.config_GTdir=null;
            gui.StartGWASpi.config_ExportDir=null;
            gui.StartGWASpi.config_ReportsDir=null;
            gui.StartGWASpi.config_OfflineHelpDir=null;
            gui.StartGWASpi.config_LogDir=null;
        }
    }
        ...
        try {
            ...
            if(_startWithGUI){   //GUI MODE
                ...
                if(dirToData.getPath().equals("")){
                    ...
                    File dataDir = null;
                    JOptionPane.showMessageDialog(gui.StartGWASpi.mainGUIFrame, global.Text.App.initText);
                    dataDir = gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION);

                    if (dataDir!=null) {
                        try {
                            if (dataDir != null) {
                                createDataStructure(dataDir);
                                JOptionPane.showMessageDialog(gui.StartGWASpi.mainGUIFrame, "Databases and working folders initialized successfully!");
                            }
                            isInitiated=true;

                        } catch (Exception ex) {
                            JOptionPane.showMessageDialog(gui.StartGWASpi.mainGUIFrame, Text.App.warnUnableToInitForFirstTime);
                            ex.printStackTrace();
                        }
                    }
                    ...
                    if(!derbyCenter.exists()){
                        int decision = gui.utils.Dialogs.showOptionDialogue("Data folder unreachable", "The data folder is unreachable (deleted?).\nShould GWASpi recreate it or do you want to provide a new path?", "Recreate", "New Path", "Cancel");
                        if(decision==JOptionPane.OK_OPTION){
                            createDataStructure(dirToData);
                            JOptionPane.showMessageDialog(gui.StartGWASpi.mainGUIFrame, "Databases and working folders initialized successfully!");
                        }
                        if(decision==JOptionPane.NO_OPTION){
                            dirToData = gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION);
                            createDataStructure(dirToData);
                            JOptionPane.showMessageDialog(gui.StartGWASpi.mainGUIFrame, "Databases and working folders initialized successfully!");
                        }
...
//                    }
                ...
                if(dirToData.getPath().equals("")){
                    if (scriptFile!=null) {
                        ...
                        File dataDir = new File(cli.Utils.readDataDirFromScript(scriptFile)); //1st line contains data path
                        if (!dataDir.exists()) {
                            dataDir = null;
                        }
    ...
    protected static void createDataStructure(File dataDir) throws IOException, BackingStoreException, URISyntaxException{
        ...
        if(!derbyCenter.exists()){
            database.DatabaseGenerator.initDataCenter();
        }

        global.Utils.createFolder(dataDir.getPath(), "genotypes");
        global.Utils.createFolder(dataDir.getPath(), "help");
        global.Utils.createFolder(dataDir.getPath(), "export");
        global.Utils.createFolder(dataDir.getPath(), "reports");
        global.Utils.createFolder(dataDir.getPath() + "/reports", "log");
        ...
        URL localVersionPath = Config.class.getClass().getResource(constants.cGlobal.LOCAL_VERSION_XML);
        ...
        setConfigValue("CURRENT_GWASPIDB_VERSION", global.XMLParser.getTextValue(localElements.get(0), "GWASpi_DB_Version"));
        
        database.StudyGenerator.createStudyLogFile(0);
    }

    protected static void updateConfigDataDirs(File dataDir) throws IOException, BackingStoreException, URISyntaxException{
        String lastOpenedDir = getConfigValue("LAST_OPENED_DIR", constants.cGlobal.HOMEDIR);
        String lastSelectedNode = getConfigValue("LAST_SELECTED_NODE", global.Text.App.appName);
        ...
        String lastVersionNb = global.Config.getConfigValue("CURRENT_GWASPIDB_VERSION", "2.0.1");
    ...
    public static void checkUpdates() throws IOException, ParseException, ParserConfigurationException, SAXException, URISyntaxException{
        if(global.Utils.checkIntenetConnection()){
            URL localVersionPath = Config.class.getClass().getResource(constants.cGlobal.LOCAL_VERSION_XML);
            Document localDom = global.XMLParser.parseXmlFile(localVersionPath.toURI().toString());
            ...
            if(localDom!=null){ //Found local version info
                ...
                URL remoteVersionPath = new URL(constants.cGlobal.REMOTE_VERSION_XML);
                Document remoteDom = global.XMLParser.parseXmlFile(remoteVersionPath.toURI().toString());
                ...
                if (remoteDom!=null) { //Found remote version info
                    ...
                    ArrayList<Element> remoteElements = global.XMLParser.parseDocument(remoteDom, "GWASpi");
                    Date remoteUpdateDate = global.XMLParser.getDateValue(remoteElements.get(0), "Date");
                    String remoteVersionNumber = global.XMLParser.getTextValue(remoteElements.get(0), "Number");
                    String remoteCompatibilityNumber = global.XMLParser.getTextValue(remoteElements.get(0), "Compatibility");
                    
                    StringBuilder message = new StringBuilder(global.Text.App.newVersionAvailable);
                    message.append("\nLocal Version: ").append(localVersionNumber);
                    message.append("\nNewest Version: ").append(remoteVersionNumber);
                    message.append("\nUpdate Type: ").append(global.XMLParser.getTextValue(remoteElements.get(0), "Type"));
                    
                    //MAKE VERSION CHECKS
                    if (remoteCompatibilityNumber.compareTo(localVersionNumber) <= 0) { //Remote version is still compatible with local version
                        message.append("\n").append(global.Text.App.newVersionIsCompatible).append("\n").append(global.XMLParser.getTextValue(remoteElements.get(0), "ActionCompatible"));
                    } else {     //Remote version is NOT compatible with local version
                        message.append("\n").append(global.Text.App.newVersionIsUnCompatible).append("\n").append(global.XMLParser.getTextValue(remoteElements.get(0), "ActionUnCompatible"));
                    }
                    message.append("\nChangelog: ").append(global.XMLParser.getTextValue(remoteElements.get(0), "Description"));
                    
                    if (localUpdateDate.compareTo(remoteUpdateDate) < 0) { //Remote version is more recent
                        if (gui.StartGWASpi.guiMode) {
                            gui.utils.Dialogs.showWarningDialogue(message.toString());
                        } else {
    ...
    public static boolean downloadFile(String dwnlUrl, String savePath, String saveName) {
      ...
      result =  global.FileDownload.download(dwnlUrl, saveFile.getPath());
      return result;
    }

...
import constants.cImport.Annotation.GWASpi;
...
public class InsertSampleInfo {
    ...
    private static String processStartTime = global.Utils.getMediumDateTimeAsString();
    ...
    public static ArrayList processData(Integer studyId, LinkedHashMap sampleInfoLHM) throws FileNotFoundException, IOException{
        ...
        db = global.ServiceLocator.getDbManager(dbName);
        try {
            ...
            for (int i=0; i<rs.size(); i++) // loop through rows of result set
            {
                ...
                if(!rs.isEmpty() && rs.get(i).size()==1){
                    samplesAllreadyInDBAL.add(rs.get(i).get(constants.cDBSamples.f_SAMPLE_ID).toString());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static ArrayList insertSamplesByHashMap(Integer studyId, LinkedHashMap sampleInfoLHM, ArrayList samplesAllreadyInDBAL) throws IOException {
        ...
        db = ServiceLocator.getDbManager(constants.cDBGWASpi.DB_DATACENTER);
        ...
        for (Iterator it = sampleInfoLHM.keySet().iterator(); it.hasNext();) {
            ...
            if (!samplesAllreadyInDBAL.contains(sampleId)) {
                ...
                db.insertValuesInTable(constants.cDBGWASpi.SCH_SAMPLES,
                                       constants.cDBSamples.T_SAMPLES_INFO,
                                       constants.cDBSamples.F_INSERT_SAMPLES_ALLINFO,
        ...
        if (result.size()>0) {
            ...
            global.Utils.logBlockInStudyDesc(operation.toString(), studyId);
            //////////////////////////////
        }
    ...
    public static int updateSamplesByHashMap(Integer studyId, LinkedHashMap sampleInfoLHM, ArrayList samplesAllreadyInDBAL) throws IOException {
        ...
        for (Iterator it = sampleInfoLHM.keySet().iterator(); it.hasNext();) {
            ...
            if (samplesAllreadyInDBAL.contains(cVals[1].toString())) {
                db.updateTable(constants.cDBGWASpi.SCH_SAMPLES,
                        constants.cDBSamples.T_SAMPLES_INFO,
                        constants.cDBSamples.F_UPDATE_SAMPLES_ALLINFO,
                        ...
                        new String[]{constants.cDBSamples.f_SAMPLE_ID, constants.cDBSamples.f_POOL_ID},

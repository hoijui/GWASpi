...
public class GenericReportGenerator_opt {
    ...
    protected static LinkedHashMap labelerHM = new LinkedHashMap();
    protected static long snpNumber = 1000000;
    protected static double threshold  = 0.5/snpNumber;  //(0.05/10⁶ SNPs => 5*10-⁷)
    ...
    protected static Color qq_back = Color.getHSBColor(0.1f, 0.0f, 0.9f);
    protected static Color qq_dot = Color.blue;
    protected static Color qq_ci = Color.lightGray;
    ...
    public static CombinedRangeXYPlot buildManhattanPlot(int opId, String netCDFVar) throws FileNotFoundException, IOException{
        ...
        threshold  = Double.parseDouble(global.Config.getConfigValue("CHART_MANHATTAN_PLOT_THRESHOLD", "5E-7"));

        String[] tmp = global.Config.getConfigValue("CHART_MANHATTAN_PLOT_BCKG", "200,200,200").split(",");
        ...
        tmp = global.Config.getConfigValue("CHART_MANHATTAN_PLOT_BCKG_ALT", "230,230,230").split(",");
        ...
        tmp = global.Config.getConfigValue("CHART_MANHATTAN_PLOT_DOT", "0,0,255").split(",");
        hsbTmp = Color.RGBtoHSB(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]), Integer.parseInt(tmp[2]), null);
...
//        }
        ...
        rdInfoMarkerSet.fillInitLHMWithVariable(constants.cNetCDF.Variables.VAR_MARKERS_CHR);
        for (Iterator it = rdInfoMarkerSet.markerIdSetLHM.keySet().iterator(); it.hasNext();) {
            Object key = it.next();
            String chr = rdInfoMarkerSet.markerIdSetLHM.get(key).toString();
        ...
        rdInfoMarkerSet.fillInitLHMWithVariable(constants.cNetCDF.Variables.VAR_MARKERS_POS);
        for (Iterator it = rdInfoMarkerSet.markerIdSetLHM.keySet().iterator(); it.hasNext();) {
            Object key = it.next();
            Object[] data = (Object[]) dataSetLHM.get(key); //CHR, POS, PVAL
            int pos = (Integer) rdInfoMarkerSet.markerIdSetLHM.get(key);
            data[1] = pos;
            dataSetLHM.put(key, data);
        }
        
        if (rdInfoMarkerSet.markerIdSetLHM!=null) {
            rdInfoMarkerSet.markerIdSetLHM.clear();
        }
        //</editor-fold>
...
//            chrAxis.setStandardTickUnits(units);
    ...
    public static XYPlot buildQQPlot(int opId, String netCDFVar, int df) throws FileNotFoundException, IOException{
        ...
        //<editor-fold defaultstate="collapsed" desc="PLOT DEFAULTS">
        String[] tmp = global.Config.getConfigValue("CHART_QQ_PLOT_BCKG", "230,230,230").split(",");
        float[] hsbTmp = Color.RGBtoHSB(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]), Integer.parseInt(tmp[2]), null);
        qq_back = Color.getHSBColor(hsbTmp[0], hsbTmp[1], hsbTmp[2]);

        tmp = global.Config.getConfigValue("CHART_QQ_PLOT_DOT", "0,0,255").split(",");
        hsbTmp = Color.RGBtoHSB(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]), Integer.parseInt(tmp[2]), null);
        qq_dot = Color.getHSBColor(hsbTmp[0], hsbTmp[1], hsbTmp[2]);

        tmp = global.Config.getConfigValue("CHART_QQ_PLOT_2SIGMA", "195,195,195").split(",");
        hsbTmp = Color.RGBtoHSB(Integer.parseInt(tmp[0]), Integer.parseInt(tmp[1]), Integer.parseInt(tmp[2]), null);
        ...
        if (df==1) {
            expChiSqrDist = statistics.Chisquare.getChiSquareDistributionDf1AL(N, 1.0f);
        } else if (df==2) {
            expChiSqrDist = statistics.Chisquare.getChiSquareDistributionDf2AL(N, 1.0f);
        }
        ...
        InputStream boundaryStream = GenericReportGenerator_opt.class.getClass().getResourceAsStream("/resources/chisqrboundary-df"+df+".txt");
    ...
    public static XYDataset getManhattanZoomByChrAndPos(int opId,
        ...
        try {
            ...
            for (Iterator it = rdAssocMarkerSetLHM.keySet().iterator(); it.hasNext();) {
                Object key = it.next();
                Object[] chrInfo = (Object[]) rdInfoMarkerSet.markerIdSetLHM.get(key);
...
//        System.out.println("\n");
            ...
            gui.reports.ManhattanPlotZoom.labelerLHM = labelerHM;
    ...
    /**
     * This getManhattanZoomByMarkerIdOrIdx has now been deprecated in favor of getManhattanZoomByChrAndPos
...
//        System.out.println("\n");
            ...
            String validateChr=rdInfoMarkerSet.markerIdSetLHM.get(origMarkerId).toString();
            gui.reports.ManhattanPlotZoom.centerPhysPos = minPosition;
    ...
    public static XYDataset getSampleHetzygDataset(int opId) throws IOException {
        ...
        ArrayList<Double> hetzygVals = rdSampleQAOPSet.getALWithVariable(sampleQANcFile, constants.cNetCDF.Census.VAR_OP_SAMPLES_HETZYRAT);
        ArrayList<Double> missingratVals = rdSampleQAOPSet.getALWithVariable(sampleQANcFile, constants.cNetCDF.Census.VAR_OP_SAMPLES_MISSINGRAT);
        ...
        snpNumber = labelerHM.size();
        gui.reports.SampleQAHetzygPlotZoom.labelerLHM = labelerHM;

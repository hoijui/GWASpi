...
public class CurrentMatrixPanel extends javax.swing.JPanel {
    ...
    public CurrentMatrixPanel(int _matrixId) throws IOException {
        ...
        matrix = new model.Matrix(_matrixId);
        ...
        txtA_MatrixDesc.setDocument(new gui.utils.JTextFieldLimit(1999));
        ...
        tbl_MatrixOperations.setModel(new javax.swing.table.DefaultTableModel(
            model.OperationsList.getOperationsTable(_matrixId),
            new String [] {
...
javax.swing.GroupLayout pnl_ButtonsLayout = new javax.swing.GroupLayout(pnl_Buttons);
    ...
    private void actionExtractMatrix(ActionEvent evt) {
        try {
            gui.GWASpiExplorerPanel.pnl_Content = new MatrixExtractPanel(matrix.getMatrixId(), "", "");
    ...
    private void actionMatrixTrafo(ActionEvent evt) {
        try {
            gui.GWASpiExplorerPanel.pnl_Content = new MatrixTrafoPanel(matrix.getMatrixId());
            gui.GWASpiExplorerPanel.scrl_Content.setViewportView(gui.GWASpiExplorerPanel.pnl_Content);
        } catch (IOException ex) {
            Logger.getLogger(CurrentMatrixPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void actionMatrixAnalyse(ActionEvent evt) {
        try {
            gui.GWASpiExplorerPanel.pnl_Content = new MatrixAnalysePanel(matrix.getMatrixId(), Integer.MIN_VALUE);
            gui.GWASpiExplorerPanel.scrl_Content.setViewportView(gui.GWASpiExplorerPanel.pnl_Content);
        } catch (IOException ex) {
            Logger.getLogger(CurrentMatrixPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void actionMatrixExport(ActionEvent evt) {
        ...
        if(format!=null){
            String expPhenotype = constants.cDBSamples.f_AFFECTION;
            if(format.equals( constants.cExport.ExportFormat.PLINK_Binary) || format.equals( constants.cExport.ExportFormat.Eigensoft_Eigenstrat)){
                try {
                    //SELECT PHENOTYPE COLUMN TO USE
                    
                    if(format.equals( constants.cExport.ExportFormat.Eigensoft_Eigenstrat)){
                        expPhenotype = gui.utils.Dialogs.showPhenotypeColumnsSelectCombo();
                    }
                    ...
                    if(markersQAOpId != Integer.MIN_VALUE){
                        ...
                        gui.utils.Dialogs.showWarningDialogue(global.Text.Operation.warnOperationsMissing+" Marker QA");
                    }
    ...
    private void actionMatrixMerge(ActionEvent evt) {
        try {
            gui.GWASpiExplorerPanel.pnl_Content = new MatrixMergePanel(matrix.getMatrixId());
            gui.GWASpiExplorerPanel.scrl_Content.setViewportView(gui.GWASpiExplorerPanel.pnl_Content);
        } catch (IOException ex) {
            Logger.getLogger(CurrentMatrixPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void saveDescription() {
        ...
        try {
            ...
            result = db.updateTable(constants.cDBGWASpi.SCH_MATRICES,
    ...
    private void actionDeleteMatrix() {
        ...
        if (threadbox.SwingWorkerItemList.permitsDeletion(null, matrix.getMatrixId(), null)) {
    ...
    private void btn_BackActionPerformed(ActionEvent evt) {
        try {
            ...
            gui.GWASpiExplorerPanel.pnl_Content = new CurrentStudyPanel(matrix.getStudyId());
            gui.GWASpiExplorerPanel.scrl_Content.setViewportView(gui.GWASpiExplorerPanel.pnl_Content);
        } catch (IOException ex) {
    ...
    private void btn_HelpActionPerformed(ActionEvent evt) {
        try {
            gui.utils.URLInDefaultBrowser.browseHelpURL(HelpURLs.QryURL.currentMatrix);
        } catch (IOException ex) {
            Logger.getLogger(CurrentMatrixPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

...
public final class SampleQAHetzygPlotZoom extends javax.swing.JPanel {
    ...
    public void initChart() throws IOException{
        ...
        hetzyThreshold = Double.parseDouble(global.Config.getConfigValue("CHART_SAMPLEQA_HETZYG_THRESHOLD", "0.5"));
        missingThreshold = Double.parseDouble(global.Config.getConfigValue("CHART_SAMPLEQA_MISSING_THRESHOLD", "0.5"));
...
//        setCursor(gui.utils.CursorUtils.waitCursor);
        ...
        String titlePlot = global.Text.Reports.smplHetzyVsMissingRat;
        ...
        lbl_thresholds.setText(global.Text.Reports.thresholds);

        lbl_hetzy.setText(global.Text.Reports.heterozygosity);
        txt_hetzy.setText(hetzyThreshold.toString());

        lbl_missing.setText(global.Text.Reports.missRatio);
        txt_missing.setText(missingThreshold.toString());
        btn_redraw.setText(global.Text.Reports.redraw);
...
//        setCursor(gui.utils.CursorUtils.defaultCursor);
    ...
    protected static XYDataset getSampleHetzygDataset(int _opId) throws IOException{
        ...
        XYDataset xyd = reports.GenericReportGenerator_opt.getSampleHetzygDataset(opId);
        return xyd;
    }
...
//            }
    ...
    private void validateThresholdAndRedraw(){
        try {
            ...
            global.Config.setConfigValue("CHART_SAMPLEQA_HETZYG_THRESHOLD", hetzyThreshold.toString());
            global.Config.setConfigValue("CHART_SAMPLEQA_MISSING_THRESHOLD", missingThreshold.toString());
            gui.GWASpiExplorerPanel.pnl_Content = new SampleQAHetzygPlotZoom(opId);
    ...
    private static void actionReset(ActionEvent evt) throws IOException {
        gui.GWASpiExplorerPanel.pnl_Content = new gui.reports.SampleQAHetzygPlotZoom(opId);
    ...
    private static void actionSaveAs(int studyId) {
        try {
            File newFile = new File(gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION).getPath()+"/SampleQA_hetzyg-missingrat_"+global.Utils.stripNonAlphaNumeric(rdMatrixMetadata.getMatrixFriendlyName())+".png");

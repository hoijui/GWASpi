...
public class Report_SampleInfoPanel extends javax.swing.JPanel {
    ...
    public Report_SampleInfoPanel(final int _studyId) throws IOException {
        ...
        tbl_ReportTable.setDefaultRenderer(Object.class, new gui.utils.RowRendererDefault());
    ...
    private void actionLoadReport() throws IOException {
        ...
        while (count < rsAllSamplesFromPool.size()) {
            ...
            if(!rsAllSamplesFromPool.isEmpty() && rsAllSamplesFromPool.get(count).size()==constants.cDBSamples.T_CREATE_SAMPLES_INFO.length){
                ...
                String familyId = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_FAMILY_ID).toString();
                String sampleId = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_SAMPLE_ID).toString();
                String fatherId = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_FATHER_ID).toString();
                String motherId = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_MOTHER_ID).toString();
                String sex = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_SEX).toString();
                String affection = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_AFFECTION).toString();
                String age = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_AGE).toString();
                String category = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_CATEGORY).toString();
                String disease = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_DISEASE).toString();
                String population = rsAllSamplesFromPool.get(count).get(constants.cDBSamples.f_POPULATION).toString();
...
//        }
    ...
    private void actionSaveReportViewAs(int studyId) {
        try
        {
            File newFile = new File(gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION).getPath()+"/sampleInfo.txt");
    ...
    private void actionHelp() {
        try {
            gui.utils.URLInDefaultBrowser.browseHelpURL(HelpURLs.QryURL.sampleInforeport);
        } catch (Exception ex) {
        }
    }

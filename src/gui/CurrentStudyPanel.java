...
public class CurrentStudyPanel extends javax.swing.JPanel {
    ...
    public CurrentStudyPanel(int _studyId) throws IOException {
        ...
        study = new model.Study(_studyId);
        DefaultMutableTreeNode matrixNode = (DefaultMutableTreeNode) gui.GWASpiExplorerPanel.tree.getLastSelectedPathComponent();
        treeChildrenLHM = gui.utils.NodeToPathCorrespondence.buildNodeToPathCorrespondence(matrixNode, false);
        ...
        tbl_MatrixTable.setDefaultRenderer(Object.class, new gui.utils.RowRendererDefault());
        ...
        tbl_MatrixTable.setModel(new javax.swing.table.DefaultTableModel(
            model.MatricesList.getMatricesTable(study.getStudyId()),
            new String [] {
    ...
    private void saveDescActionPerformed() {
        try {
            global.Utils.logBlockInStudyDesc(txtA_StudyDesc.getText(), study.getStudyId());
    ...
    private void actionLoadSampleInfo() throws IOException {
        gui.utils.Dialogs.showInfoDialogue(Text.Study.infoSampleInfo);
        File sampleInfoFile = gui.utils.Dialogs.selectFilesAndDirertoriesDialogue(JOptionPane.OK_OPTION);
            ...
            if (option == JOptionPane.YES_OPTION) {
                ...
                if (option == JOptionPane.YES_OPTION && deleteReportOption != JOptionPane.CANCEL_OPTION) {
                    ...
                    for (int i = 0; i < selectedMatrices.length; i++) {
                        ...
                        if (threadbox.SwingWorkerItemList.permitsDeletion(null, matrixId, null)) {
    ...
    private void actionDeleteStudy(ActionEvent evt) {
        //TEST IF THE DELETED ITEM IS REQUIRED FOR A QUED WORKER
        if (threadbox.SwingWorkerItemList.permitsDeletion(study.getStudyId(), null, null)) {
    ...
    private void btn_HelpActionPerformed(ActionEvent evt) {
        try {
            gui.utils.URLInDefaultBrowser.browseHelpURL(HelpURLs.QryURL.currentStudy);
        } catch (IOException ex) {
            Logger.getLogger(CurrentMatrixPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

...
import threadbox.SwingDeleterItemList;
import threadbox.SwingWorkerItem;
import threadbox.SwingWorkerItemList;
...
public class ProcessTab extends javax.swing.JPanel {
        ...
        if(gui.StartGWASpi.guiMode){
            ...
            tmpTable.addMouseListener(new MouseListener() {
                public void mouseClicked(MouseEvent e) {
                    ...
                    if (colIndex == 7) {    //Abort
                        if(rowIndex<threadbox.SwingWorkerItemList.getSwingWorkerItemsALsize()){
                            threadbox.SwingWorkerItemList.flagCurrentItemAborted(rowIndex);
                        } else {
                            threadbox.SwingDeleterItemList.abortSwingWorker(rowIndex-threadbox.SwingWorkerItemList.getSwingWorkerItemsALsize());
                        }
            ...
            tmpTable.setDefaultRenderer(Object.class, new gui.utils.RowRendererProcessOverviewWithAbortIcon());
    ...
    public static void toggleBusyLogo(){
        ...
        while(count<swingWorkerItemsAL.size()){
            ...
            if(!queueState.equals(threadbox.QueueStates.DONE) &&
               !queueState.equals(threadbox.QueueStates.ABORT) &&
               !queueState.equals(threadbox.QueueStates.ERROR)){
                idle = false;
            } else {
                idle = true;
            }
    ...
    public static void showTab() {
        gui.StartGWASpi.allTabs.setSelectedIndex(gui.StartGWASpi.allTabs.getTabCount()-1);
        startBusyLogo();
    }
    ...
    private void actionSaveAs() {
        try {
            File newFile = new File(gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION).getPath()+"/process.log");

...
public class LoadDataPanel extends javax.swing.JPanel {
    ...
    public LoadDataPanel(int _studyId) {
        ...
        cmb_Format.setModel(new javax.swing.DefaultComboBoxModel(constants.cImport.ImportFormat.values()));
        ...
        btn_Go.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                try {
                    ...
                    try {
                        gui.utils.Dialogs.showWarningDialogue(global.Text.App.warnProcessInterrupted);
                        System.out.println(global.Text.App.warnProcessInterrupted);
                        
                        //DELETE BROKEN NEW MATRIX AND REPORTS
                        MatrixMetadata deleteMxMetaData = netCDF.matrices.MatrixManager.getLatestMatrixId();
                        if(deleteMxMetaData.getMatrixFriendlyName().equals(txt_NewMatrixName.getText())){
                            ...
                            netCDF.matrices.MatrixManager.deleteMatrix(deleteMxMetaData.getMatrixId(), true);
                        }
                    ...
                    try {
                        ex.printStackTrace();
                        gui.utils.Dialogs.showWarningDialogue(global.Text.All.warnLoadError+"\n"+global.Text.All.warnWrongFormat);
                        System.out.println(global.Text.All.warnLoadError);
                        System.out.println(global.Text.All.warnWrongFormat);
...
//                );
            ...
            if (!newMatrixName.isEmpty()) {
                ...
                if(filesOK[0]==true && filesOK[1]==true && filesOK[2]==true){
                    ...
                    if (decision == JOptionPane.YES_OPTION) {
                        //ASK MORE QUESTIONS
                        gwasParams = gui.utils.MoreGWASinOneGoInfo.showGWASInOneGo_Modal(cmb_Format.getSelectedItem().toString());
                ...
                setCursor(gui.utils.CursorUtils.defaultCursor);
        ...
        private void actionBrowse1(java.awt.event.ActionEvent evt) {
            ...
            if(cmb_Format.getSelectedItem().equals(cImport.ImportFormat.PLINK)){
                ...
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File1, txt_File1, "");
                if (!txt_File1.getText().isEmpty()) {
                    ...
                    if (txt_File2.getText().isEmpty() && pedFile.exists()) {
                        ...
                        if (pedFile.exists()) {
                            int option = gui.utils.Dialogs.showConfirmDialogue(global.Text.Matrix.findComplementaryPlink);
                ...
                if (!txt_File1.getText().isEmpty()) {
                    ...
                    if (txt_File2.getText().isEmpty() && bimFile.exists()) {
                        ...
                        if (bimFile.exists()) {
                            option = gui.utils.Dialogs.showConfirmDialogue(global.Text.Matrix.findComplementaryPlinkBinary);
                            if (option == JOptionPane.YES_OPTION) {
                                txt_File2.setText(bimFile.getPath());
                            }
                        }
                ...
                gui.utils.Dialogs.selectAndSetDirectoryDialogue(evt, btn_File1, txt_File1, "", ""); //only dirs
            } else {
                //Use standard file opener
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File1, txt_File1, "");
            }
        ...
        private void actionBrowse2(java.awt.event.ActionEvent evt) {
            //Use standard file opener
            if(cmb_Format.getSelectedItem().equals(constants.cImport.ImportFormat.PLINK)){
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File2, txt_File2, "");
            ...
            } else if(cmb_Format.getSelectedItem().equals(constants.cImport.ImportFormat.PLINK_Binary)){
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File2, txt_File2, "");
                if (!txt_File2.getText().isEmpty()) {
            ...
            } else if(cmb_Format.getSelectedItem().equals(constants.cImport.ImportFormat.BEAGLE)) {
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File2, txt_File2, "");
            } else if(cmb_Format.getSelectedItem().equals(constants.cImport.ImportFormat.HGDP1)) {
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File2, txt_File2, "");
            } else if(cmb_Format.getSelectedItem().equals(cImport.ImportFormat.Sequenom)){
                gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_File2, txt_File2, "");
            } else {
                gui.utils.Dialogs.selectAndSetDirectoryDialogue(evt, btn_File2, txt_File2, "", ""); //only dirs
            }
        }

        private void actionBrowseSampleInfo(java.awt.event.ActionEvent evt) {
            //Use standard file opener
            gui.utils.Dialogs.selectAndSetFileDialogue(evt, btn_FileSampleInfo, txt_FileSampleInfo, "");
        }
            ...
            if(txt_File1.isEnabled()){
                if (file1.exists()) {
                    ...
                    if(fieldObligatoryState[0]==false){
                        if(txt_File1.getText().contains(global.Text.All.optional) ||
            ...
            if(txt_File2.isEnabled()){
                if(file2.exists()){
                    ...
                    if(fieldObligatoryState[1]==false){
                        if(txt_File2.getText().contains(global.Text.All.optional) ||
            ...
            if (txt_FileSampleInfo.isEnabled()) {
                if (file3.exists()) {
                    ...
                    if(fieldObligatoryState[2]==false){
                        if(txt_FileSampleInfo.getText().contains(global.Text.All.optional) ||
        ...
        private void btn_BackActionPerformed(java.awt.event.ActionEvent evt) {
            try {
                gui.GWASpiExplorerPanel.pnl_Content = new CurrentStudyPanel(studyId);
        ...
        private void actionGWASHelp() {
            try {
                gui.utils.URLInDefaultBrowser.browseHelpURL(HelpURLs.QryURL.loadGts);
            } catch (IOException ex) {

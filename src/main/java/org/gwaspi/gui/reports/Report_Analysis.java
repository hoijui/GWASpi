...
public abstract class Report_Analysis extends javax.swing.JPanel {
	...
	private void actionSaveReportViewAs(int studyId, String chartPath) {
		try {
			String newPath = org.gwaspi.gui.utils.Dialogs.selectDirectoryDialogue(JOptionPane.OK_OPTION).getPath() + "/" + txt_NRows.getText() + "rows_" + chartPath;
	...
	private void actionHelp() {
		try {
			org.gwaspi.gui.utils.URLInDefaultBrowser.browseHelpURL(HelpURLs.QryURL.assocReport);
		} catch (Exception ex) {
		}
	}
	...
	protected void displayColumnCursor(MouseEvent me) {
		...
		if (!getCursor().equals(org.gwaspi.gui.utils.CursorUtils.waitCursor)) {
			if (columnName.equals(Text.Reports.zoom)) {
				setCursor(org.gwaspi.gui.utils.CursorUtils.handCursor);

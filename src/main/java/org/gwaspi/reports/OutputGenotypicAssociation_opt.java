...
public class OutputGenotypicAssociation_opt {
	...
	public static boolean writeReportsForAssociationData(int opId) throws FileNotFoundException, IOException {
		...
		if (writeManhattanPlotFromAssociationData(opId, manhattanName, 4000, 500)) {
			...
			ReportManager.insertRPMetadata(dBManager,
					...
					op.getStudyId());
			System.out.println("Saved Genotypic Association Manhattan Plot in reports folder at " + org.gwaspi.global.Utils.getMediumDateTimeAsString());
		}
		...
		if (result && writeQQPlotFromAssociationData(opId, qqName, 500, 500)) {
			...
			System.out.println("Saved Genotypic Association QQ Plot in reports folder at " + org.gwaspi.global.Utils.getMediumDateTimeAsString());
		}
	...
	public static boolean createSortedAssociationReport(int opId, String reportName) throws FileNotFoundException, IOException {
		...
		try {
			LinkedHashMap unsortedMarkerIdAssocValsLHM = GenericReportGenerator_opt.getAnalysisVarData(opId, org.gwaspi.constants.cNetCDF.Association.VAR_OP_MARKERS_ASGenotypicAssociationTP2OR);

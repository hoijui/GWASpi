...
public class SystemSort {
    ...
    public static void processData(File dir, String format, JProgressBar progressBar) throws IOException{
        File[] filesToImport = global.Utils.listFiles(dir.toString(), false);
        File formatFolder = global.Utils.createFolder(global.Config.getConfigValue("ESdir", constants.cGlobal.USERDIR), format);
        String processingDir = formatFolder.getPath() + "/";
        File[] alreadySortedFiles = global.Utils.listFiles(processingDir, false);
        numFiles = alreadySortedFiles.length;
        
        for (int i=0; i<filesToImport.length; i++) {
            String cmd = getCommandLine(filesToImport[i].getPath(), format);
            global.SysCommandExecutor.sysCommandExecute(cmd);
    ...
    public static String getCommandLine(String pathToFile, String format) throws IOException{
        String executable;
        String commandLine = "";
        String outputPath = global.Config.getConfigValue("ESdir", constants.cGlobal.USERDIR).toString()+"/"+format+"/"+(numFiles-1)+".csv";
        numFiles++;
